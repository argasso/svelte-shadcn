...
const singleValueQueryStoreCache: Record<string, Writable<string>> = {}
...
const queryStoreCache: Record<string, Writable<string[]>> = {}
...
function createQueryStore(prop: string): Writable<string[]> {
  ...
  page.subscribe((p) => {
    path = p.url.pathname
    query = browser ? p.url.searchParams : new URLSearchParams() // For prerender
